...
%% load bag file
pitch_bag = ros.Bag.load('vz_2.bag');
input_msg_all = pitch_bag.readAll({'/q4/real/cmd_vel_stamped'});
input_msg = input_msg_all(2:231);
...
figure;
hold all;
plot(input_state(:, 1), input_state(:, 16), '-r');
plot(input_state(:, 1), input_state(:, 7), '-b');

%% store the data
save('vz_2.mat', 'input_state');
