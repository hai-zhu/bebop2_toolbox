...
%% load bag file
pitch_bag = ros.Bag.load('roll_2.bag');
...
for i = 1 : input_samples
    ...
    % position
    input_state(i, 2) = state_msg{index_now}.state.y;              % x
    input_state(i, 3) = state_msg{index_now}.state.x * (-1);       % y
    input_state(i, 4) = state_msg{index_now}.state.z;              % z
    % velocity
    input_state(i, 5) = state_msg{index_now}.state.y_dot;
    input_state(i, 6) = state_msg{index_now}.state.x_dot * (-1);
    input_state(i, 7) = state_msg{index_now}.state.z_dot;
    % euler angles
    input_state(i, 8) = state_msg{index_now}.state.roll;           % pitch
    input_state(i, 9) = state_msg{index_now}.state.pitch;          % roll
    input_state(i, 10) = state_msg{index_now}.state.yaw;           % yaw
    input_state(i, 11) = state_msg{index_now}.state.roll_dot;
    input_state(i, 12) = state_msg{index_now}.state.pitch_dot;
...
figure;
hold all;
axis([-3 3 -3 3 0.2 2.5]);
plot3(input_state(:, 2), input_state(:, 3), input_state(:, 4));

%% plot angle
figure;
hold all;
plot(input_state(:, 1), input_state(:, 15).*180/pi, '-r');
plot(input_state(:, 1), input_state(:, 9).*180/pi, '-b');

%% store the data
save('roll_2.mat', 'input_state');
